...
function MemberUnlock({
  ...
  return (
    <div style={{
      ...
      border: "1px solid " + (member ? "rgba(120,220,160,0.35)" : "var(--border)"),
      borderRadius: 14, textAlign: "left",
    }}>
      <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", marginBottom: 4 }}>
        {member ? "✨ " + (membership!.plan || "Mystic Plus") + " active" : "Unlock Premium readings"}
      </p>
...
export default function ReadingPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/membership")
      .then((r) => {
        if (r.status === 401) {
          if (active) { setGuest(true); setMembership(null); }
          return null;
        }
  ...
  async function startReading() {
    ...
    try {
      const r = await fetch("/api/reading", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // Only the question is sent. Premium eligibility is decided server-side
        // from the authenticated session — never from a client-supplied email.
