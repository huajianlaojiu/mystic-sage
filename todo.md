...
**3b. 更基础的错误：我用临时文件传递密钥去测试，结果把自己骗了。**
我把 `.env.local` 的 service_role key 写进临时 JSON，再用 PowerShell 读回来测，
得到 401，于是下结论"这个密钥已失效"。后来**直接读 `.env.local` 测同一个接口，
返回 200 并读到真实数据——密钥根本没问题**。

**教训：验证凭证时不要经过中间文件或编码转换，直接读原始值测试。
拿到 401 时先怀疑测试方法，再怀疑凭证。**

> 修正：本次曾断言"用户给的 `sb_secret_...` 令牌已失效"，**该断言错误**。
> 那个密钥是有效的。它之所以改不了邮箱验证开关，是因为
> **service_role 密钥与 Management API 令牌是两套体系**（前者读写数据，
> 后者改项目配置），和是否失效无关。
